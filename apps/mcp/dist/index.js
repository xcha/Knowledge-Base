...
server.registerTool('search_knowledge', {
    description: '在 AI 知识库中进行语义检索，返回与查询最相关的文档片段。适合查找具体知识点、事实或文档内容。',
    inputSchema: {
    ...
    const res = await fetch(`${API_BASE}/knowledge/${KB_ID}/search?q=${encodeURIComponent(query)}&topK=${topK}`, {
    ...
    const text = data.chunks
        .map((c, i) => `[片段${i + 1}] 来源：${c.documentName}（相似度 ${(c.score * 100).toFixed(1)}%）\n${c.content}`)
        .join('\n\n---\n\n');
    return { content: [{ type: 'text', text }] };
});
// 工具2：列出知识库文档
server.registerTool('list_documents', {
    description: '列出知识库中所有文档的名称和基本信息。适合了解知识库整体内容或确认某文档是否存在。',
    inputSchema: {},
}, async () => {
    ...
    const text = data
        .map((d) => `- ${d.originalName}（${d._count.chunks} 个片段，上传于 ${new Date(d.createdAt).toLocaleDateString('zh-CN')}）`)
        .join('\n');
